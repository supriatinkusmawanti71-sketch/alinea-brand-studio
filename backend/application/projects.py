...
SUPPORTED_REDO_STAGES = frozenset({"INTAKE", "DIRECTIONS", "LOGO", "IP"})
# Stages regenerated by seeding upstream state into a fresh workflow thread
# instead of replaying the whole graph (which would re-bill earlier stages).
_SEEDED_REDO_SPECS = {
    "LOGO": {
        "upstream_stage": "DIRECTIONS",
        "upstream_key": "direction_version_id",
        "as_node": "await_direction_decision",
        "missing_selection_error": "缺少已选方向，无法重新生成 Logo",
    },
    "IP": {
        "upstream_stage": "LOGO",
        "upstream_key": "logo_version_id",
        "as_node": "await_logo_decision",
        "missing_selection_error": "缺少已选 Logo，无法重新生成 IP",
    },
}
...
async def request_stage_control(
    ...
    *,
    ...
    if project.status == "COMPLETED" and action != "REDO":
    ...
    raise UnsupportedStageControlError(
        f"{action} is not supported for {stage}; the current flow runs IP automatically after Logo"
    )
...
async def _redo_stage(
    ...
    *,
    ...
    input_json: dict[str, object] = {"redo": redo_payload, "decision_id": decision_id}
    seed_spec = _SEEDED_REDO_SPECS.get(source_version.stage)
    if seed_spec is not None:
        upstream_version_id = source_version.input_refs_json.get(seed_spec["upstream_key"])
        if not isinstance(upstream_version_id, str):
            raise StageControlConflictError(
                f"{source_version.stage} version is missing upstream references"
            )
        upstream_version = await session.get(StageVersion, upstream_version_id)
        if (
            upstream_version is None
            or upstream_version.project_id != source_version.project_id
            or upstream_version.status != "GENERATED"
        ):
            raise StageControlConflictError("上游版本已失效，请先重新完成上一阶段的选择")
        # The version's own decision_id may point at a REDO decision (when this
        # version was itself regenerated), so resolve the selected item from
        # the upstream version's selection decision instead.
        selection_decision = await session.scalar(
            select(Decision)
            .where(
                Decision.source_version_id == upstream_version_id,
        ...
        selected_item_id = (
            selection_decision.selected_item_id if selection_decision is not None else None
        )
        if not selected_item_id:
            raise StageControlConflictError(seed_spec["missing_selection_error"])
        seed_json: dict[str, object] = {
            "as_node": seed_spec["as_node"],
            "selected_stage": seed_spec["upstream_stage"],
            "upstream_version_id": upstream_version_id,
            "selected_item_id": selected_item_id,
        }
        if reason:
            seed_json["feedback"] = reason
        input_json["seed"] = seed_json
        input_json[seed_spec["upstream_key"]] = upstream_version_id
    redo_run = StageRun(
        id=run_id,
        workflow_thread_id=run_id,
